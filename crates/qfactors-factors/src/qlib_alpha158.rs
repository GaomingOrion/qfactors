...
//!   qfactors emits `NaN` over each symbol's first `N-1` warmup rows.
//! - `IMAX`/`IMIN` add `+1` because qfactors `ts_argmax`/`ts_argmin` are 0-based
...
mod tests {
    ...
    use qfactors_core::expr::collect_fields;
