...
pub(crate) fn build_time_index(df: &DataFrame, panel: &PanelOptions) -> Result<TimeIndex> {
    let symbol = df
        ...
        .map_err(|_| EvalError::Core(QFactorsError::MissingColumn(panel.symbol_col.clone())))?;
    let time = df
        .column(&panel.time_col)
        .map_err(|_| EvalError::Core(QFactorsError::MissingColumn(panel.time_col.clone())))?;
    ...
    if !(&time_changed | &symbol_changed).all() {
        return Err(EvalError::Core(QFactorsError::DuplicateSymbolTime {
...
fn validate_structural(column: &Column, is_symbol: bool) -> Result<()> {
    if column.null_count() > 0 {
        return Err(EvalError::Core(if is_symbol {
            QFactorsError::SymbolNull(column.name().to_string())
        } else {
            QFactorsError::TimeNull(column.name().to_string())
        }));
    }
    if matches!(column.dtype(), DataType::Float32 | DataType::Float64) {
        let has_nan = column.as_materialized_series().is_nan()?.any();
        if has_nan {
            return Err(EvalError::Core(QFactorsError::NaNNotAllowed {
...
mod tests {
    ...
    fn rejects_duplicates_and_structural_nulls() {
        ...
        assert!(matches!(
            err,
            EvalError::Core(QFactorsError::DuplicateSymbolTime { .. })
        ));
        ...
        assert!(matches!(err, EvalError::Core(QFactorsError::SymbolNull(_))));
        ...
        assert!(matches!(
            err,
            EvalError::Core(QFactorsError::NaNNotAllowed { .. })
        ));
    }
}
